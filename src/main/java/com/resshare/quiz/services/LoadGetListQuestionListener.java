...
public class LoadGetListQuestionListener extends QuizBaseListener {
...
//	}
	...
	public String getReferenceName() {
		...
		return ResFirebaseReference.getInputPathReference("../post_get_question");
...
//	}
	...
	public void onChildAdded(DataSnapshot snapshot1, String previousChildName) {
		try {
			if (snapshot1.child("processing").getValue() == null) {
				...
				String user_id = (String) snapshot1.child("user_id").getValue(String.class);
			//	if(user_id==null)
			//	String	user_id = (String) snapshot1.child("user_id_destination").getValue(String.class);
				String subject = (String)snapshot1.child("data/subject").getValue(String.class);
			...
			//	ResponseClient.sendResponse(objJs);
				...
				FirebaseDatabase.getInstance().getReference(ResFirebaseReference.getAbsolutePath("../data/" + questions_type +"/"+subject +"/"+ questions_key))
						.addListenerForSingleValueEvent(new ValueEventListener() {
							...
							public void onDataChange(DataSnapshot snapshotquestions) {
								...
								if (snapshotquestions.exists()) {
									...
									if(snapshot1.hasChild("parent_message_key"))
									{
									 
									String parent_message_key = (String) snapshot1.child("parent_message_key").getValue(String.class);
									
									Map mapControlRecycler_viewCommand = new HashMap<>();
									mapControlRecycler_viewCommand.put(CommandConstant.recycler_remove_row, parent_message_key);
									...
									sendMessageQuestionView( snapshot1,  user_id, questions_name_master,
											arrayQuestionsNameNext, objSnapshotquestions_name_detail,examination_key,1,subject,questions_type,questions_key);
									// question_1
	...
	public void sendMessageQuestionView2(  DataSnapshot snapshot11,  String user_id,
