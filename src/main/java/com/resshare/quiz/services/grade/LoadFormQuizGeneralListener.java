...
public class LoadFormQuizGeneralListener extends QuizBaseListener {
		...
		try {
			if (snapshot1.child("processing").getValue() == null) {
				...
				FirebaseDatabase.getInstance()
						.getReference(ResFirebaseReference.getAbsolutePath("../data/profile/" + user_id+"/grade"))
							...
							public void onDataChange(DataSnapshot snapshot2) {
								if (snapshot2.exists()) {
									if (snapshot2.getValue() != null) {
										...
										viewGridUIGradeListQuestion(snapshot1,snapshot2.getValue(String.class),subject);
										return;
									}

								}
								viewGridUIGrade(snapshot1);
