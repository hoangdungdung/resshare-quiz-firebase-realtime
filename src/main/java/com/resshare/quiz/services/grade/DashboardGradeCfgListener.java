...
public class DashboardGradeCfgListener extends QuizBaseListener {
...
//	}
	...
	public String getReferenceName() {
		...
		return ResFirebaseReference.getInputPathReference("../grade_cfg");
	...
	public void onChildAdded(DataSnapshot snapshot1, String previousChildName) {
		try {
			if (snapshot1.child("processing").getValue() == null) {
				...
				if(snapshot1.hasChild("message_key"))
				{
				 
				String message_key = (String) snapshot1.child("message_key").getValue(String.class);
				
				Map mapControlRecycler_viewCommand = new HashMap<>();
				mapControlRecycler_viewCommand.put(CommandConstant.recycler_remove_row, message_key);
				...
				sendResponse(msgDashboardMessage);
			 	...
			 	viewGridUIGradeListQuestion(snapshot1,gradeValue,subject);
